...
final class OnActionExtensionTests: XCTestCase {
    ...
    /// When the wrap returns early, the original closure must NOT fire — otherwise the fallthrough
    /// side effect would still run.
    func testWrapPatternSkipsPreviousClosureOnEarlyReturn() {
        ...
        control.onAction = { _ in
            // simulate an intercept branch: do side effect, return early
